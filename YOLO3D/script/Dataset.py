...
class Dataset(data.Dataset):
    def __init__(self, path, bins=2, overlap=0.1):
        ...
        self.ids = [x.split('.')[0] for x in sorted(os.listdir(self.top_calib_path)) if x.endswith(".txt") and len(x.split('.')[0]) > 0]
        ...
        class_list = ['small_black_alu', 'allenkey', 'large_black_alu', 'large_grey_alu','motor2', 'screwdriver', 'housing', 'screw','axis2','bearing2','small_nut','large_nut','spacer','blue_container','drill','red_container','wrench','small_grey_alu']
    ...
    def __getitem__(self, index):
        ...
        if self.proj_matrix is None:
            self.proj_matrix = np.array([[637.59228515625, 0, 320.0],
                                         [0, 637.59228515625, 240.0],
                                         [0,    0,   1]])
    ...
    def format_label(self, line):
        ...
        [6.375923e+02, 0.000000e+00, 3.200000e+02],
        [0.000000e+00, 6.375923e+02, 2.400000e+02],
        ...
        if dx < 0:
            mult = -1
        dx = abs(dx)
        theta_ray = np.arctan((2*dx*np.tan(fovx/2)) / width)
        theta_ray= theta_ray * mult
        #_______ ende Neu
