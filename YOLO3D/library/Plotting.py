...
# Updated function to handle yaw, pitch, and roll
def plot_3d_box(img, cam_to_img, ry, dimension, center, pitch=0, roll=0):
    
    # Create full rotation matrix with yaw, pitch, and roll
    R = rotation_matrix(ry, pitch, roll)
    ...
    _draw_3d_box_edges(img, box_3d)


def _draw_3d_box_edges(img, box_3d):
    """
    Helper function to draw the 3D box edges given the projected corner points
    """
    #LINE CONNECTIONS FOR 3D BOX
    # Bottom face (indices 0,2,6,4 form bottom rectangle)
    ...
    # Top face (indices 1,3,7,5 form top rectangle)
    ...
    # Optional: Fill front face with transparency
    frame = np.zeros_like(img, np.uint8)
    # front side (you may need to adjust which face is considered "front" based on your coordinate system)
